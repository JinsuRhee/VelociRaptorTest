...
;-)
	...
	;; Keyword setting
	;;-----
	;if ~keyword_set(dir_catalog) then print, '	***** Set Dir_catalog'
	;if ~keyword_set(dir_catalog) then stop

	;if ~keyword_set(n_snap) then print, "***** Put the snapshot number into n_snap argument"
	;if ~keyword_set(n_snap) then stop
	...
	dir_snap= dir_catalog + 'snap_t2_' + string(n_snap,format='(I3.3)') + '/'
	...
	if keyword_set(verbose) then tic
	if keyword_set(verbose) then print, '        %%%%%                           '
	if keyword_set(verbose) then print, '        %%%%% Particle Property         '
	if keyword_set(verbose) then print, '        %%%%%                           '

	if strlen(file_search(dir_snap + 'rv_prop.sav')) lt 5L or keyword_set(rv_prop) then begin
		if keyword_set(verbose) then print, '        %%%%% (No previous works are found)'

		if ~keyword_set(num_thread) then spawn, 'nproc --all', num_thread
		if ~keyword_set(num_thread) then num_tread = long(num_thread)

		rv_prop, output, output2, dir_snap=dir_snap, dir_raw=dir_raw, dir_lib=dir_lib, horg=horg, num_thread=num_thread, $
			n_snap=n_snap, flux_list=flux_list

		save, filename=dir_snap + 'rv_prop.sav', output2
	endif else begin
	        restore, dir_snap + 'rv_prop.sav'
	endelse
	output	= rv_makestr(output2, output=output)
	if keyword_set(verbose) then print, '        %%%%% Done in                      '
	if keyword_set(verbose) then print, ' '
	...
	if strlen(file_search(dir_snap + 'rv_gprop.sav')) lt 5L or keyword_set(rv_gprop) then begin
		...
		rv_gprop, output, output2, dir_snap=dir_snap, dir_raw=dir_raw, dir_lib=dir_lib, horg=horg, num_thread=num_thread, $
			n_snap=n_snap
