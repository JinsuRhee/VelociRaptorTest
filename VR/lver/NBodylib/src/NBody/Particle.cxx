...
namespace NBody
{
    ...
    bool PIDCompareVec(const Particle&a, const Particle &b)
    {
        //Int_t aa = a.GetPID();
        //Int_t bb = b.GetPID();
        long long aa = a.GetPID();
        long long bb = b.GetPID();
    ...
    {
	    //if(p.id > 22080) cout<<"%123 Operator "<<p.id<<" / "<<sizeof(type)<<" / "<<sizeof(p.type)<<" / "<<sizeof(p.rho)<<" / "<<sizeof(rho)<<endl;
        if (this!=&p) {
#ifndef NOMASS
            mass = p.mass;
#endif
	    //cout<<"%123 Operator "<<p.id<<" / "<<sizeof(pid)<<" / "<<sizeof(p.pid)<<endl;
            position[0] = p.position[0];
            position[1] = p.position[1];
            position[2] = p.position[2];
...
*/
    ...
    void Particle::Write(std::ostream &F) const
    {
        ...
        F.write((char*)&id, sizeof(PARTIDTYPE));
