...
int RAMSES_fortran_read(fstream &F, RAMSESIDTYPEL *i){
    int dummy,byteoffset=0;
    F.read((char*)&dummy, sizeof(dummy));byteoffset+=sizeof(long long);
    F.read((char*)i,dummy); byteoffset+=dummy;
    F.read((char*)&dummy, sizeof(dummy));byteoffset+=sizeof(long long);
    return byteoffset;
}
...
long long RAMSES_get_nbodies(char *fname, int ptype, Options &opt)
{
    ...
    */
	...
	Framses.read((char*)&dummy, sizeof(dummy));
        ...
        {
		if((dummy_family[j] == 1) && (fabs((dummy_mass[j]-dmp_mass)/dmp_mass) < 1e-5)) ramses_header_info.npart[RAMSESDMTYPE]++;
...
void ReadRamses(Options &opt, vector<Particle> &Part, const long long nbodies, Particle *&Pbaryons, long long nbaryons)
{
    char buf[2000],buf1[2000],buf2[2000];
	...
	//Fpartfam[i].read((char*)&famtempchunk2, dummy);
	//Fpartfam[i].read((char*)&dummy, sizeof(dummy));

	for(int ii2=0; ii2<chunksize; ii2 ++) famtempchunk[ii2] = famtempchunk2[ii2] + 0;
	//if(i<10) for(int nn3=0; nn3<10;nn3++)cout<<"%123 ROOT TEST:"<<nn3<<" / "<<i<<" : "<<famtempchunk[nn3]<<" / "<<chunksize<<" / "<<sizeof(famtempchunk[nn3])<<endl;
	//if(i==1) for(int nn3=0; nn3<10;nn3++)cout<<"%123 ROOT TEST2:"<<idvalchunk[chunksize-nn3-1]<<" / "<<chunksize<<endl;
        //RAMSES_fortran_read(Fpartid[i],idvalchunk);
...
//             for (int kk=0;kk<3;kk++) {xtemp[kk]=LittleRAMSESFLOAT(xtemp[kk]);vtemp[kk]=LittleRAMSESFLOAT(vtemp[kk]);}
	    ...
	    typeval = 100;
	    if (fabs((mtemp-dmp_mass)/dmp_mass) < 1e-5 && famtempchunk[nn] == 1) typeval = DARKTYPE;
