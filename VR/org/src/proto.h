...
long long ReadHeader(Options &opt);
///Reads particle data
void ReadData(Options &opt, vector<Particle> &Part, const long long nbodies, Particle *&Pbaryons, long long nbaryons=0);
///Read gadget file
void ReadGadget(Options &opt, vector<Particle> &Part, const long long nbodies,Particle *&Pbaryons, long long nbaryons=0);
///Read tipsy file
void ReadTipsy(Options &opt, vector<Particle> &Part, const long long nbodies,Particle *&Pbaryons, long long nbaryons=0);
#ifdef USEHDF
///Read HDF format
void ReadHDF(Options &opt, vector<Particle> &Part, const long long nbodies,Particle *&Pbaryons, long long nbaryons=0);
#endif
///Read ramses file
void ReadRamses(Options &opt, vector<Particle> &Part, const long long nbodies,Particle *&Pbaryons, long long nbaryons=0);
///Read Nchilada file
void ReadNchilada(Options &opt, vector<Particle> &Part, const long long nbodies,Particle *&Pbaryons, long long nbaryons=0);
///Adjust hydro particles/quantities to appropriate units
void AdjustHydroQuantities(Options &opt, vector<Particle> &Part, const long long nbodies);
///Adjust star particles/quantities to appropriate units
void AdjustStarQuantities(Options &opt, vector<Particle> &Part, const long long nbodies);
///Adjust BH particles/quantities to appropriate units
void AdjustBHQuantities(Options &opt, vector<Particle> &Part, const long long nbodies);

///Read local velocity density
void ReadLocalVelocityDensity(Options &opt, const long long nbodies, vector<Particle> &Part);
///Writes local velocity density of each particle to a file
void WriteLocalVelocityDensity(Options &opt, const long long nbodies, vector<Particle> &Part);


///Writes a tipsy formatted fof.grpfile
void WriteFOF(Options &opt, const long long nbodies, Int_t *pfof);
...
void WriteExtendedOutput(Options &opt, Int_t numgroups, long long nbodies, PropData *pdata, vector<Particle> &p, Int_t * pfof);
...
//@{
Int_t ReadPFOF(Options &opt, long long nbodies, Int_t *pfof);
Int_t ReadFOFGroupBinary(Options &opt, long long nbodies, Int_t *pfof, Int_t *idtoindex, Int_t minid, Particle *p);
//@}

///Reads properties of a subvolume
void ReadCellValues(Options &opt, const long long nbodies, const long long ngrid, GridCell *grid, Coordinate *gvel, Matrix *gveldisp);
// ///Writes cell quantites
//void WriteCellValues(Options &opt, const long long nbodies, const long long ngrid, GridCell *grid, Coordinate *gvel, Matrix *gveldisp);
...
void WriteExtendedOutput (Options &opt, Int_t numgroups, long long nbodies, PropData *pdata, Particle *p, Int_t * pfof);
#endif
//@}
...
KDTree* InitializeTreeGrid(Options &opt, const long long nbodies, Particle *Part);
///Fill cells of grid from tree
void FillTreeGrid(Options &opt, const long long nbodies, const long long ngrid, KDTree *&tree, Particle *Part, GridCell* &grid);
...
Coordinate *GetCellVel(Options &opt, const long long nbodies, Particle *Part, long long ngrid, GridCell *grid);
///Calculate velocity dispersion tensor of cell
Matrix *GetCellVelDisp(Options &opt, const long long nbodies, Particle *Part, long long ngrid, GridCell *grid, Coordinate *gvel);
//@}
...
void GetVelocityDensity(Options &opt, const long long nbodies, Particle *Part, KDTree *tree=NULL);
///sub interfaces depending on type of velocity density desired.
void GetVelocityDensityOld(Options &opt, const long long nbodies, Particle *Part, KDTree *tree);
///Velocity density where only particles in a halo (which is localised to an mpi domain) are within the tree
void GetVelocityDensityHaloOnlyDen(Options &opt, const long long nbodies, Particle *Part, KDTree *tree);
///exact velocity density, finds for each particle nearest physical neighbours and estimates velocity
void GetVelocityDensityExact(Options &opt, const long long nbodies, Particle *Part, KDTree *tree);
///optimised search for cosmological simulations
void GetVelocityDensityApproximative(Options &opt, const long long nbodies, Particle *Part, KDTree *tree);
//@}
...
void GetDenVRatio(Options &opt, const long long nbodies, Particle *Part, long long ngrid, GridCell *grid, Coordinate *gvel, Matrix *gveldisp);
///Characterize the ratio distribution to estimate intrinsic scatter in this estimator
void DetermineDenVRatioDistribution(Options &opt,const long long nbodies, Particle *Part, Double_t &meanr,Double_t &sdlow,Double_t &sdhigh, int subleve=0);
///Calculate normalized residual
Int_t GetOutliersValues(Options &opt, const long long nbodies, Particle *Part, int sublevel=0);
...
Int_t *SearchFullSet(Options &opt, const long long nbodies, vector<Particle> &Part, Int_t &numgroups);
///Search the outliers
Int_t *SearchSubset(Options &opt, const long long nbodies, const Int_t nsubset, Particle *Partsubset, Int_t &numgroups, Int_t sublevel=0, Int_t *pnumcores=NULL);
...
Int_t* SearchBaryons(Options &opt, long long &nbaryons, Particle *&Pbaryons, const long long ndark, vector<Particle> &Partsubset, Int_t *&pfofdark, Int_t &ngroupdark, Int_t &nhalos, int ihaloflag=0, int iinclusive=0, PropData *phalos=NULL);
...
void AdjustStructureForPeriod(Options &opt, const long long nbodies, vector<Particle> &Part, Int_t numgroups, Int_t *pfof);
...
int CheckUnboundGroups(Options opt, const long long nbodies, Particle *Part, Int_t &ngroup, Int_t *&pfof, Int_t *numingroup=NULL, Int_t **pglist=NULL,int ireorder=1, Int_t *groupflag=NULL);
...
void Potential(Options &opt, long long nbodies, Particle *Part, Double_t *potV);
void Potential(Options &opt, long long nbodies, Particle *Part);
void PotentialPP(Options &opt, long long nbodies, Particle *Part);
//@}
...
void ScaleLinkingLengths(Options &opt,const long long nbodies, Particle *Part,Coordinate &cm,Coordinate &cmvel,Double_t Mtot);
///Adjust to cm of halo
void AdjusttoCM(const long long nbodies, Particle *Part, Coordinate &cm, Coordinate &cmvel, const Double_t Mtot, Double_t *rlim, Double_t &MaxVcirc, Double_t tol=1e-2);
///Calculate virial radius and related quantities
void GetVirialQuantities(const long long nbodies, Particle *Part, const Double_t mtot, Double_t *rlim, const Double_t rhoc, const Double_t virlevel, Double_t &Rvir, Double_t &Mvir, Int_t nenc, Double_t *Menc, Double_t *Renc);
...
void GetCM(Options &opt, const long long nbodies, Particle *Part, Int_t ngroup, Int_t *&pfof, Int_t *&numingroup, PropData *&pdata, Int_t *&noffset);
///Get the properties of the substructures and output the results
void GetProperties(Options &opt, const long long nbodies, Particle *Part, Int_t ngroup, Int_t *&pfof, Int_t *&numingroup, PropData *&pdata, Int_t *&noffset);
///Get CM properties
//void GetCMProp(Options &opt, const long long nbodies, Particle *Part, Int_t ngroup, Int_t *&pfof, Int_t *&numingroup, PropData *&pdata, Int_t *&noffset);
///Get max dist to several reference frames
void GetMaximumSizes(Options &opt, long long nbodies, Particle *Part, Int_t ngroup, Int_t *&numingroup, PropData *&pdata, Int_t *&noffset);
...
void GetInclusiveMasses(Options &opt, const long long nbodies, Particle *Part, Int_t ngroup, Int_t *&pfof, Int_t *&numingroup, PropData *&pdata, Int_t *&noffset);
///Get FOF masses for field objects
void GetFOFMass(Options &opt, const long long nbodies, Particle *Part, Int_t ngroup, Int_t *&pfof, Int_t *&numingroup, PropData *&pdata, Int_t *&noffset);
...
void GetSOMasses(Options &opt, const long long nbodies, Particle *Part, Int_t ngroup, Int_t *&numingroup, PropData *&pdata);
...
///Get Binding Energy
void GetBindingEnergy(Options &opt, const long long nbodies, Particle *Part, Int_t ngroup, Int_t *&pfof, Int_t *&numingroup, PropData *&pdata, Int_t *&noffset);

///Get Morphology properties (since this is for a particular system just use pointer interface)
void GetGlobalSpatialMorphology(const long long nbodies, Particle *p, Double_t& q, Double_t& s, Double_t Error, Matrix& eigenvec, int imflag=0, int itype=-1, int iiterate=1);
...
Int_t **SortAccordingtoBindingEnergy(Options &opt, const long long nbodies, Particle *Part, Int_t ngroup, Int_t *&pfof, Int_t *numingroup, PropData *pdata, Int_t ioffset=0);
///used to calculate properties and ignores keeping particle order, assumes particle PID information meaningless
void CalculateHaloProperties(Options &opt, const long long nbodies, Particle *Part, Int_t ngroup, Int_t *&pfof, Int_t *numingroup, PropData *pdata);
...
Int_t OpenMPLocalSearch(Options &opt,
    const long long nbodies, vector<Particle> &Part, Int_t * &pfof, Int_t *&storeorgIndex,
...
OMP_ImportInfo *OpenMPImportParticles(Options &opt, const long long nbodies, vector<Particle> &Part, Int_t * &pfof, Int_t *&storetype,
...
void OpenMPLinkAcross(Options &opt,
    long long nbodies, vector<Particle> &Part, Int_t * &pfof,
...
Int_t OpenMPResortParticleandGroups(long long nbodies, vector<Particle> &Part, Int_t *&pfof, Int_t minsize);

///sets the head/next arrays based on the current particle order and the current pfof array
void OpenMPHeadNextUpdate(const long long nbodies, vector<Particle> &Part, const Int_t numgroups, Int_t *&pfof, Int_tree_t *&Head, Int_tree_t *&Next);
#endif
...
void MPIDomainDecompositionZOOMIN(int n1, int n2, Double_t, Double_t);
...
void MPIAddParticletoAppropriateBuffer(Options &opt, const int &ibuf, Int_t ibufindex, int *&ireadtask, const Int_t &Bufsize, Int_t *&Nbuf, Particle *&Pbuf, long long &numpart, Particle *Part, Int_t *&Nreadbuf, vector<Particle>*&Preadbuf);
...
void MPISendReceiveHydroInfoBetweenThreads(Options &opt, Int_t nlocalbuff, Particle *Pbuf, long long nlocal, Particle *Part, int recvTask, int tag, MPI_Comm &mpi_comm);
///Send/Receive star information between read threads using the MPI communicator
void MPISendReceiveStarInfoBetweenThreads(Options &opt, Int_t nlocalbuff, Particle *Pbuf, long long nlocal, Particle *Part, int recvTask, int tag, MPI_Comm &mpi_comm);
///Send/Receive BH information between read threads using the MPI communicator
void MPISendReceiveBHInfoBetweenThreads(Options &opt, Int_t nlocalbuff, Particle *Pbuf, long long nlocal, Particle *Part, int recvTask, int tag, MPI_Comm &mpi_comm);
///Send/Receive BH information between read threads using the MPI communicator
void MPISendReceiveExtraDMInfoBetweenThreads(Options &opt, Int_t nlocalbuff, Particle *Pbuf, long long nlocal, Particle *Part, int recvTask, int tag, MPI_Comm &mpi_comm);
...
void MPISendReceiveFOFHydroInfoBetweenThreads(Options &opt, long long nexport, fofid_in *FoFGroupDataExport, long long nlocal, fofid_in *FoFGroupDataLocal, Particle *&Part, int recvTask, int tag, MPI_Comm &mpi_comm);
///Send/Receive star information between read threads using the MPI communicator
///Using a FOF filled buffer
void MPISendReceiveFOFStarInfoBetweenThreads(Options &opt, long long nexport, fofid_in *FoFGroupDataExport, long long nlocal, fofid_in *FoFGroupDataLocal, Particle *&Part, int recvTask, int tag, MPI_Comm &mpi_comm);
///Send/Receive BH information between read threads using the MPI communicator
///Using a FOF filled buffer
void MPISendReceiveFOFBHInfoBetweenThreads(Options &opt, long long nexport, fofid_in *FoFGroupDataExport, long long nlocal, fofid_in *FoFGroupDataLocal, Particle *&Part, int recvTask, int tag, MPI_Comm &mpi_comm);
///Send/Receive ExtraDM information between read threads using the MPI communicator
///Using a FOF filled buffer
void MPISendReceiveFOFExtraDMInfoBetweenThreads(Options &opt, long long nexport, fofid_in *FoFGroupDataExport, long long nlocal, fofid_in *FoFGroupDataLocal, Particle *&Part, int recvTask, int tag, MPI_Comm &mpi_comm);
//@}
...
short_mpi_t *MPISetTaskID(const long long nbodies);
...
void MPIAdjustLocalGroupIDs(const long long nbodies, Int_t *pfof);
///Determine number of particles that need to be exported to another mpi thread from local mpi thread based on rdist
void MPIGetExportNum(const long long nbodies, Particle *Part, Double_t rdist);
#ifdef SWIFTINTERFACE
...
void MPIGetExportNumUsingMesh(Options &opt, const long long nbodies, Particle *Part, Double_t rdist);
#endif
///Determine and send particles that need to be exported to another mpi thread from local mpi thread based on rdist
void MPIBuildParticleExportList(Options &opt, const long long nbodies, Particle *Part, Int_t *&pfof, Int_tree_t *&Len, Double_t rdist);
...
void MPIBuildParticleExportListUsingMesh(Options &opt, const long long nbodies, Particle *Part, Int_t *&pfof, Int_tree_t *&Len, Double_t rdist);
///Link groups across MPI threads using a physical search
Int_t MPILinkAcross(const long long nbodies, KDTree *&tree, Particle *Part, Int_t *&pfof, Int_tree_t *&Len, Int_tree_t *&Head, Int_tree_t *&Next, Double_t rdist2);
///Link groups across MPI threads using criterion
Int_t MPILinkAcross(const long long nbodies, KDTree *&tree, Particle *Part, Int_t *&pfof, Int_tree_t *&Len, Int_tree_t *&Head, Int_tree_t *&Next, Double_t rdist2, FOFcompfunc &cmp, Double_t *params);
///Link groups across MPI threads checking particle types
Int_t MPILinkAcross(const long long nbodies, KDTree *&tree, Particle *Part, Int_t *&pfof, Int_tree_t *&Len, Int_tree_t *&Head, Int_tree_t *&Next, Double_t rdist2, FOFcheckfunc &check, Double_t *params);
///update export list after after linking across
void MPIUpdateExportList(const long long nbodies, Particle *Part, Int_t *&pfof, Int_tree_t *&Len);
///localize groups to a single mpi thread
Int_t MPIGroupExchange(Options &opt, const long long nbodies, Particle *Part, Int_t *&pfof);
///Determine the local number of groups and their sizes (groups must be local to an mpi thread)
Int_t MPICompileGroups(Options &opt, const long long nbodies, Particle *Part, Int_t *&pfof, Int_t minsize);
///similar to \ref MPIGroupExchange but optimised for separate baryon search, assumes only looking at baryons
Int_t MPIBaryonGroupExchange(Options &opt, const long long nbodies, Particle *Part, Int_t *&pfof);
///similar to \ref MPICompileGroups but optimised for separate baryon search, assumes only looking at baryons
Int_t MPIBaryonCompileGroups(Options &opt, const long long nbodies, Particle *Part, Int_t *&pfof, Int_t minsize, int iorder=1);
///localize baryons particle members of groups to a single mpi thread
///Collect FOF from all
void MPICollectFOF(const long long nbodies, Int_t *&pfof);
...
void MPIBuildParticleExportBaryonSearchList(Options &opt, const long long nbodies, Particle *Part, Int_t *&pfof, Int_t *ids, Int_t *numingroup, Double_t rdist);
///search local baryons with exported particle list.
Int_t MPISearchBaryons(const long long nbaryons, Particle *&Pbaryons, Int_t *&pfofbaryons, Int_t *numingroup, Double_t *localdist, Int_t nsearch, Double_t *param, Double_t *period);
///localize the baryons to the mpi thread on which their associated DM group exists.
Int_t MPIBaryonExchange(const long long nbaryons, Particle *&Pbaryons, Int_t *&pfofbaryons);
//@}
...
void MPIBuildGridData(const long long ngrid, GridCell *grid, Coordinate *gvel, Matrix *gveldisp);
...
void MPIGetNNExportNum(const long long nbodies, Particle *Part, Double_t *rdist);
#ifdef SWIFTINTERFACE
///Determine number of particles that need to be exported to another mpi thread from local mpi thread based on array of distances for each particle for NN search
void MPIGetNNExportNumUsingMesh(Options &opt, const long long nbodies, Particle *Part, Double_t *rdist);
#endif
...
void MPIBuildParticleNNExportList(const long long nbodies, Particle *Part, Double_t *rdist);
#ifdef SWIFTINTERFACE
///Determine and send particles that need to be exported to another mpi thread from local mpi thread based on array of distances for each particle for NN search
void MPIBuildParticleNNExportListUsingMesh(Options &opt, const long long nbodies, Particle *Part, Double_t *rdist);
#endif
///Determine number of local particles that need to be exported back based on ball search.
void MPIGetNNImportNum(const long long nbodies, KDTree *tree, Particle *Part, int iallflag=1);
///Determine local particles that need to be exported back based on ball search.
Int_t MPIBuildParticleNNImportList(Options &opt, const long long nbodies, KDTree *tree, Particle *Part, int iallflag=1);
///comparison function to order particles for export
...
void MPIGetHaloSearchImportNum(const long long nbodies, KDTree *tree, Particle *Part);
///Builds the import list of particles based on halo positions
Int_t MPIBuildHaloSearchImportList(Options &opt, const long long nbodies, KDTree *tree, Particle *Part);
#ifdef SWIFTINTERFACE
...
Int_t *BuildNumInGroup(const long long nbodies, const Int_t numgroups, Int_t *pfof);
///build group size array of particles of a specific type
Int_t *BuildNumInGroupTyped(const long long nbodies, const Int_t numgroups, Int_t *pfof, Particle *Part, int type);
///build array such that array is pglist[group][]={particle list}
Int_t **BuildPGList(const long long nbodies, const Int_t numgroups, Int_t *numingroup, Int_t *pfof);
...
Int_t **BuildPGListTyped(const long long nbodies, const Int_t numgroups, Int_t *numingroup, Int_t *pfof, Particle *Part, int type);
///build the group particle index list (doesn't assume particles are in ID order and stores index of particle)
Int_t **BuildPGList(const long long nbodies, const Int_t numgroups, Int_t *numingroup, Int_t *pfof, Particle *Part);
///build pglist but doesn't assume particles are in ID order
Int_t **BuildPGList(const long long nbodies, const Int_t numgroups, Int_t *numingroup, Int_t *pfof, Int_t *ids);
...
Int_tree_t *BuildHeadArray(const long long nbodies, const Int_t numgroups, Int_t *numingroup, Int_t **pglist);
///build the Next array which points to the next particle in the group
Int_tree_t *BuildNextArray(const long long nbodies, const Int_t numgroups, Int_t *numingroup, Int_t **pglist);
///build the Len array which stores the length of the group a particle belongs to
Int_tree_t *BuildLenArray(const long long nbodies, const Int_t numgroups, Int_t *numingroup, Int_t **pglist);
///build the GroupTail array which stores the Tail of a group
Int_tree_t *BuildGroupTailArray(const long long nbodies, const Int_t numgroups, Int_t *numingroup, Int_t **pglist);
...
Int_t *BuildNoffset(const long long nbodies, Particle *Part, Int_t numgroups,Int_t *numingroup, Int_t *sortval, Int_t ioffset=0);
