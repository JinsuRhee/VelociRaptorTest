...
*/
	...
	// See whether OMP region is created or not. do not split node if interparticle distance is quite large enough
	if (size <= b2){
		js_ompskip=1;
		if(size > 1000000){
    ...
    //Node Masking
    void KDTree::BuildNodeMask(Node *node, Double_t js_fdist)
    {
	    if(node->GetLeaf()<0){
		    BuildNodeMask(((SplitNode*)node)->GetLeft(), js_fdist);
		    BuildNodeMask(((SplitNode*)node)->GetRight(), js_fdist);
	    }
	    else{
		    JS_FindLeafNode((LeafNode*)node, root, js_fdist);
	    }
    }

    //Node Masking - Find Leaf Nodes within the linking length
    void KDTree::JS_FindLeafNode(LeafNode *tarnode, Node *searnode, Double_t js_fdist)
    {
	    if(searnode->GetLeaf()<0){
		    JS_FindLeafNode(tarnode, ((SplitNode*)searnode)->GetLeft(), js_fdist);
		    JS_FindLeafNode(tarnode, ((SplitNode*)searnode)->GetRight(), js_fdist);
	    }
	    else{
	    	Double_t tar_cen[6];
		Double_t sear_cen[6];
		Double_t tar_ptcl[6];
		Double_t sear_ptcl[6];
		Int_t tar_ind0, sear_ind0;
		tar_ind0 = tarnode->GetStart();
		sear_ind0 = searnode->GetStart();

		for(int js_i=0; js_i<MAXND; js_i++){
			tar_cen[js_i] = tarnode->GetCenter(js_i);
			sear_cen[js_i] = searnode->GetCenter(js_i);
			tar_ptcl[js_i] = bucket[tar_ind0].GetPhase(js_i);
			sear_ptcl[js_i] = bucket[sear_ind0].GetPhase(js_i);
		}


		Double_t dist_cen2 = DistanceSqd(tar_cen, sear_cen, MAXND);
		Double_t dist_tar2 = DistanceSqd(tar_cen, tar_ptcl, MAXND);
		Double_t dist_sear2= DistanceSqd(sear_cen, sear_ptcl, MAXND);

		if(sqrt(dist_cen2) < sqrt(dist_tar2) + sqrt(dist_sear2) + js_fdist){
			tarnode->SetNearLeaf(searnode);
		}
