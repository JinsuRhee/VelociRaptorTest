...
*/
	...
	/////
	// -- JS --
	// Sorting first & Determine Split dimension whose interparticle distance is largest
	/////
	int js_ind0 = start;
	int js_ind1 = end-1;
	double js_indx = 0., js_indx2;
	int js_nn = (end - start) / 8;
	int js_initialdim;
	for(int js_inddim=0; js_inddim<ND; js_inddim ++){
		js_initialdim = js_inddim;
		js_qsort(js_ind0, js_ind1, js_inddim);
	        for(int js_ind=start + js_nn; js_ind<end - js_nn; js_ind++){
			js_indx2 = abs(bucket[js_ind+1].GetPhase(js_inddim) - bucket[js_ind].GetPhase(js_inddim));
			if(js_indx2 > js_indx){
				js_indx = js_indx2;
				splitdim = js_inddim;
			}
	...
	if(js_initialdim != splitdim) js_qsort(js_ind0, js_ind1, splitdim);
	...
	if (treetype==TPHYSF || treetype==TPHS){
		Double_t js_dx3 = 0.;
		for(int js_nn2=0; js_nn2<ND; js_nn2 ++){
			if(abs(js_bnd[js_nn2][1] - js_bnd[js_nn2][0]) > js_dx3){
				js_dx3 = abs(js_bnd[js_nn2][1] - js_bnd[js_nn2][0]);
	       	...
	       	if(js_dx3 < 2.0*js_rdist) goto AdditionalLeafBuild;
	}
	...
	// Normal Construction
	    ...
	    if(treetype == TOMP && size > 1000000){
		    if(js_indx > 2.0*js_rdist) goto OmpSkipBuildNode;
		    //double js_dx=0.;
