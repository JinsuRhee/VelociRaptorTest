...
namespace NBody
{
	...
	//-- JS -- Build Sibling
	void BuildSibling(Node *node);
	void BuildNodeMask(Node *node, Double_t js_fdist);
	void JS_FindLeafNode(LeafNode *tarnode, Node *searnode, Double_t js_fdist);
	void ClearNodeMask(Node *node);
	Double_t JS_GetOverrapVolume(Double_t js_linkbdn[6][2], Double_t js_bdn[6][2]);
	...
	void FOFSearch_js(Node *node, Double_t rd, Double_t fdist2, Int_t iGroup, Int_t nActive, Particle *bucket, Int_t *Group, Int_tree_t *Len, Int_tree_t *Head, Int_tree_t *Tail, Int_tree_t *Next, short *BucketFlag, Int_tree_t *Fifo, Int_t &iTail, Double_t* off, Int_t target);
        ...
        /// for positions
	inline void JSGetBoundary(int j, Int_t start, Int_t end, Double_t *bnd, KDTreeOMPThreadPool &);
