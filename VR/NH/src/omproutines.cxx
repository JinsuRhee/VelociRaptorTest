    ...
    const Int_t numompregions, OMP_Domain *&ompdomain)
{
    ...
    double time00=MyGetTime();
    int js_count=0;

    /////
    for (i=0; i<numompregions;i++) {
	    cout<<" -123123 X : "<<i<<" : "<<ompdomain[i].bnd[0][0]<<" ~ "<<ompdomain[i].bnd[0][1]<<" // "<<ompdomain[i].ncount<<" // "<<tree3dfofomp[i]->GetNumLeafNodes()<<endl;
	...
	js_count ++;
		...
		cout<<"			Task / Index / Done / NOMP / Nptcls / Ngroups / Nleafs / Elapsed Time"<<endl;
		cout<<"			"<<ThisTask<<" / "<<i<<" / "<<js_count<<" / "<<numompregions<<" / "<<ompdomain[i].ncount<<" / "<<ng<<" / "<<tree3dfofomp[i]->GetNumLeafNodes()<<" / "<<MyGetTime() - time00<<endl;
    }
    }
...
OMP_ImportInfo *OpenMPImportParticles(Options &opt, const long long nbodies, vector<Particle> &Part,
...
void OpenMPLinkAcross(Options &opt,
    long long nbodies, vector<Particle> &Part, Int_t * &pfof, Int_t *&storeorgIndex,
    Int_tree_t *&Head, Int_tree_t *&Next,
    ...
    Int_t *&omp_nrecv_total, Int_t *&omp_nrecv_offset, OMP_ImportInfo* &ompimport)
{
	...
	// maximum linking ptcls
	Int_t avg_nrecv = 0;
	for (i=0;i<numompregions;i++) {
		avg_nrecv = avg_nrecv + omp_nrecv_total[i];
	}
	avg_nrecv = avg_nrecv / numompregions;

    //cout<<ThisTask<<" finished linking "<<MyGetTime()-time1<<endl;
    cout<<ThisTask<<" finished linking "<<MyGetTime()-time1<<endl;
    cout<<"	---------- Linking Information ----------"<<endl;
    cout<<"	%123 Ptcls in each OMP domain : "<<ompdomain[0].ncount<<endl;
    cout<<"	%123 # of Linked ptcls"<<endl;
    for(i=0;i<numompregions;i++) cout<<"		- "<<omp_nrecv_total[i]<<endl;
    cout<<"	%123 AVG : "<<avg_nrecv<<" for the MPI -"<<ThisTask<<endl;
    cout<<"	-----------------------------------------"<<endl;
